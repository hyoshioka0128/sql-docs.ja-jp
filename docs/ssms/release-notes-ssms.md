...
|Azure SQL Database|Azure SQL Database でマスターではなくユーザー データベースに接続したときに、Azure SQL Database クエリ ウィンドウにデータベース リストが正しく入力されない問題を修正しました。|
|Azure SQL Database|Azure SQL Database に "テンポラル テーブル" を追加できない問題を修正しました。|
|Azure SQL Database|Azure の [統計] メニューの下にある統計プロパティのサブメニュー オプションを有効にしました。これは、もうだいぶ前から完全にサポートされていたためです。|
