...
## <a name="overview-of-using-sql-server-audit"></a>SQL Server 監査の使用の概要  
 ...
 Windows イベント ログを閲覧するには、Windows の **イベント ビューアー** ユーティリティを使用します。 ファイル ターゲットの場合は、 [!INCLUDE[ssManStudioFull](../../../includes/ssmanstudiofull-md.md)] の **[ログ ファイルの表示]** か、 [fn_get_audit_file](../../../relational-databases/system-functions/sys-fn-get-audit-file-transact-sql.md) 関数を使用して、対象のファイルを閲覧できます。  
